...
package cn.lhfei.camel.service;
...
public class NumbersService {
  ...
  /**
   * The routing participants that have subscribed to routing and have provided their rules for
   * suitable exchanges.
   */
  ...
  /**
   * The service that contains participants' results of processing the messages that they have
   * received.
   */
  private final ResultsService resultsService;

  /**
   * The {@link CountDownLatch} to wait for the total expected number of messages to be received.
   */
   ...
   */
  public NumbersService(final ExampleConfig config, final List<RoutingParticipant> participants,
      final ProducerTemplate start, final ResultsService resultsService,
      final CountDownLatch countDownLatch) {
  ...
  /**
   * After the application is started and ready, tell each routing participant to subscribe, and
   * then send the messages. Afterward, display the results and exit the app.
   */
  ...
  public void start() throws InterruptedException {
    ...
    LOG.info("Sending {} messages to the dynamic router: {}", numberFormat.format(numberOfMessages),
        getStartUri());
