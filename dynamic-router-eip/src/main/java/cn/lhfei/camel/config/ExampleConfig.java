 ...
 * @created Oct 31, 2023
 */
@ConfigurationProperties(prefix = "camel.spring-boot.dynamic-router-eip")
public class ExampleConfig {
  ...
  /**
   * The URI where messages will be sent to as the starting point for the route that feeds the
   * dynamic router.
   */
  ...
  /**
   * If the recipient mode is "firstMatch", then this is the number of messages that are expected to
   * be received. This value is also used to create a {@link CountDownLatch} so that all messages
   * can be received before printing out messaging statistics.
   */
  private int expectedFirstMatchMessageCount;

  /**
   * If the recipient mode is "allMatch", then this is the number of messages that are expected to
   * be received. There is probably not an easy way to calculate this number, so the number needs to
   * be known for a given {@link #sendMessageCount} value. This value is also used to create a
   * {@link CountDownLatch} so that all messages can be received before printing out messaging
   * statistics.
   */
